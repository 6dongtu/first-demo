<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>手机号:</label>
        <input type="text" placeholder="请输入你的手机号" v-model="phone" name="phone" v-validate="{required:true,regex:/^1\d{10}$/}"
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="password" placeholder="请输入确认密码" v-model="confirmPassword" name="password1" v-validate="{required:true,is:password}" :class="{invalid:errors.has('password1')}">
    ...
    methods: {
      async getCode(){
        ...
        if(this.phone){
          ...
          this.code = this.$store.state.LogAndReg.code
        }
        } catch (error) {
          alert(error.message)
        }
      ...
      async register(){
        try {
        // 发送请求注册
        const {phone,code,password,confirmPassword,isAgree} = this
        if(phone && code && password == confirmPassword && isAgree && errors.count()==0 ){
